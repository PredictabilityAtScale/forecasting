...
namespace VotingSystems
{
    public class VotingBase
    {
            ...
            foreach (var vote in votes)
            {
                ...
                for (int j = 0; j < vote.PreferanceOrder.Length - 1; j++) // candidate order (skip the last, we just want pairs)
                {
                    ...
                    if (jChar != '>' && jChar != '=')
                    {
                        ...
                        // compiler warning error. without this, equal found was reported as never used. BUT IT IS! This removes the warning...
                        if (equalFound) { equalFound = false; }
        ...
        public virtual XElement AsXML(List<VoteTally> votes)
        {
            return null;
        }
