...
const rescheduleBooking = createServerFn({ method: 'POST' })
  .inputValidator((data: unknown) => {
    const value = data as { token?: string; slotId?: string; durationMinutes?: unknown }
    ...
    return { token: value.token, slotId: value.slotId, durationMinutes: value.durationMinutes }
  })
  .handler(async ({ data }) => {
    const { getBookingAvailability: getCurrentBookingAvailability } = await import(
      '#/data/booking-availability'
    )
    const slot = getCurrentBookingAvailability({ durationMinutes: data.durationMinutes }).find(
      (item) => item.id === data.slotId,
    )
      ...
      <section className="island-shell rounded-3xl px-6 py-8 sm:px-8 sm:py-10">
        ...
        {action === 'reschedule' ? (
          <div className="space-y-3">
            <fieldset>
              <legend className="mb-2 block text-sm font-medium">Meeting length</legend>
